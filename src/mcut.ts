...
class Box {
  ...
  data: number[]; // it's all about the data
  box: MinMax[]; // the bounding box of the data
  dim: number; // number of dimensions in the data

  /** Initializes the data values, number of dimensions in the data (currently fixed to 3 to handle RGB, but may be
   *  genericized in the future), and the bounding box of the data.
   */
  ...
  /**
   * keeps running tally of the min and max values on each dimension initialize the min value to the highest number
   * possible, and the max value to the lowest number possible
   */
  calculate_bounding_box() {
    ...
    for (let i = this.data.length - 1; i >= 0; i -= 1) {
      minmax[0].min = this.data[i][0] < minmax[0].min ? this.data[i][0] : minmax[0].min; // r
      minmax[1].min = this.data[i][1] < minmax[1].min ? this.data[i][1] : minmax[1].min; // g
      minmax[2].min = this.data[i][2] < minmax[2].min ? this.data[i][2] : minmax[2].min; // b

      minmax[0].max = this.data[i][0] > minmax[0].max ? this.data[i][0] : minmax[0].max; // r
      minmax[1].max = this.data[i][1] > minmax[1].max ? this.data[i][1] : minmax[1].max; // g
      minmax[2].max = this.data[i][2] > minmax[2].max ? this.data[i][2] : minmax[2].max; // b
    }
  ...
  /**
   * Return a comparison function based on a given index (for median-cut, sort on the longest axis) ie: sort ONLY on a
   * single axis.  get_comparison_func( 1 ) would return a sorting function that sorts the data according to each item's
   * Green value.
   */
  get_comparison_func(_i: number): ComparisonFunction {

    return (a, b) => a[_i] - b[_i];
  }

  /**
   * Sorts all the elements in this box based on their values on the longest axis.
   */
  sort() {

    const a = this.get_longest_axis().axis;
    const sort_method = this.get_comparison_func(a);
  ...
  /**
   * Returns the position of the median value of the data in this box.  The position number is rounded down, to deal
